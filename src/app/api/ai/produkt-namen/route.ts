...
const bodySchema = z.object({
  artikelnummer: z.string().min(1).max(200),
  bereichName: z.string().max(200).optional().nullable(),
  kategorieName: z.string().max(200).optional().nullable(),
  infoKurz: z.string().max(1000).optional().nullable(),
  technischeDaten: z.record(z.string(), z.string().max(200)).optional().nullable(),
...
function checkRateLimit(key: string): { ok: boolean; remaining: number } {
  ...
  if (entry.count >= LIMIT_PER_HOUR) return { ok: false, remaining: 0 };
  entry.count++;
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await generateProduktNamen(parsed.data, { provider, model, apiKey });
